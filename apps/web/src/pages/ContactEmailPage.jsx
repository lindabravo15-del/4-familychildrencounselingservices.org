...
const ContactEmailPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.full_name || !form.email || !form.message) {
      setError(t.contactEmailPage.errorRequired);
      return;
    }
    setError('');
    setStatus('submitting');
    try {
      await pocketbaseClient.collection('contact_email_submissions').create(form);
      setStatus('success');
    } catch (err) {
      setStatus('idle');
      setError(t.contactEmailPage.errorGeneric);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#e9f3ee] via-[#eaf2f5] to-[#f3f0e6] text-[#3a5a6b]">
      <Helmet>
        <title>Contact Us | Family Counseling &amp; Child Therapy in Rialto, CA | 4-Family &amp; Children Counseling Services</title>
        <meta name="description" content="Contact 4-Family & Children Counseling Services. Reach out for family therapy, child counseling, teen therapy, or parenting help in Rialto, Riverside County, and San Bernardino County. Call (951) 906-0176." />
      </Helmet>
        ...
        >
          <div className="flex items-center justify-center gap-3 mb-4">
            ...
            <h1 className="font-display text-3xl font-bold text-[#3a5a6b] md:text-4xl">
              {t.contactEmailPage.title}
            </h1>
            <span className="h-px w-10 bg-[#c9d6cf]" />
          </div>
          <p className="text-center text-[#5a7183]">
            {t.contactEmailPage.subtitle}
          </p>

          <div className="mt-6 flex items-start gap-3 rounded-xl bg-white/70 px-5 py-4 shadow-sm">
            <ShieldCheck className="mt-0.5 h-5 w-5 shrink-0 text-[#6ba368]" />
            <p className="text-sm leading-relaxed text-[#4a6273]">
              {t.contactEmailPage.confidentiality}
            </p>
          </div>
          ...
          {status === 'success' ? (
            <motion.div
              ...
              className="mt-8 flex flex-col items-center gap-4 rounded-2xl bg-white px-8 py-14 text-center shadow-md"
            >
              <CheckCircle2 className="h-14 w-14 text-[#7bb06e]" />
              <h2 className="font-display text-2xl font-bold text-[#3a5a6b]">{t.contactEmailPage.successTitle}</h2>
              <p className="text-[#5a7183]">
                {t.contactEmailPage.successMessage}
              </p>
              <Link
                to="/"
              ...
              >
                {t.contactEmailPage.returnHome}
              </Link>
            </motion.div>
          ) : (
            <form
              onSubmit={handleSubmit}
              className="mt-8 flex flex-col gap-6 rounded-2xl bg-white px-6 py-8 shadow-md md:px-10 md:py-10"
            >
              <div className="flex flex-col gap-2">
                <label htmlFor="full_name" className="text-sm font-semibold text-[#3a5a6b]">
                  {t.contactEmailPage.fullName}
                </label>
                <input
                  id="full_name"
                  type="text"
              ...
              <div className="flex flex-col gap-2">
                <label htmlFor="message" className="text-sm font-semibold text-[#3a5a6b]">
                  {t.contactEmailPage.message} <span className="text-[#9ab0a2] font-normal">{t.contactEmailPage.messageHint}</span>
                </label>
                <textarea
                  id="message"
                  rows={4}
                  ...
                  placeholder={t.contactEmailPage.messagePlaceholder}
              ...
              >
                ...
                {status === 'submitting' ? t.contactEmailPage.sending : t.contactEmailPage.submit}
              </button>
            </form>
          )}
        </motion.div>
